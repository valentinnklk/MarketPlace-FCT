...
class ReseñaModelo {
    ...
    /**
     * Valoraciones recibidas por un servicio concreto (no por todos los del
     * prestador). Útil en la vista de detalle del servicio para que los clientes
     * decidan basándose en experiencias previas con ese mismo servicio.
     */
    public function getReseñasPorServicio(int $servicio_id, int $limite = 20): array {
        $stmt = $this->conn->prepare(
            "SELECT v.id, v.puntuacion, v.comentario, v.fecha,
                    u.nombre     AS autor,
                    u.avatar_url AS autor_avatar
    ...
    /**
     * Estadísticas agregadas (media y total) de un servicio concreto.
     */
    public function getEstadisticasServicio(int $servicio_id): array {
        $stmt = $this->conn->prepare(
            "SELECT COUNT(*) AS total, AVG(v.puntuacion) AS media
             FROM valoraciones v
             JOIN contratos c ON c.id = v.contrato_id
             WHERE c.servicio_id = ?"
        );
        ...
        return [
            'total' => (int) ($r['total'] ?? 0),
            'media' => $r['total'] ? round((float) $r['media'], 1) : 0.0,
        ];
    }
}
