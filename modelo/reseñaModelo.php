...
//
// Reseñas sobre la tabla REAL `reseñas` del proyecto:
//   reseñas(id, contrato_id UNIQUE, revisor_id, revisado_id,
//           puntuacion tinyint 1-5, comentario, fecha)
//
// Regla de negocio: solo se puede crear una reseña si el contrato
// asociado está en estado 'completado' y todavía no tiene reseña.
//
// Al crear una reseña actualizamos también `servicios.valoracion_media`
...
class ReseñaModelo {
    ...
    /**
     * Comprueba si un cliente puede reseñar un contrato.
    ...
    public function puedeReseñar(int $contrato_id, int $cliente_id): array {
        $stmt = $this->conn->prepare(
            "SELECT c.id, c.cliente_id, c.estado, c.servicio_id,
                    ...
                    (SELECT r.id FROM reseñas r WHERE r.contrato_id = c.id LIMIT 1) AS reseña_id
             FROM contratos c
    ...
    /**
     * Crea una reseña y actualiza la valoración media del servicio.
     ...
     * con la media de todas las reseñas recibidas por el prestador
    ...
    private function recalcularMediaServicio(int $servicio_id): void {
        $stmt = $this->conn->prepare(
            "UPDATE servicios s
                LEFT JOIN (
                    SELECT c.servicio_id, ROUND(AVG(r.puntuacion), 1) AS media
                    FROM reseñas r
    ...
    /**
     * Datos de un contrato + reseña asociada (si la hubiera).
    ...
    public function getContratoConReseña(int $contrato_id): array|false {
        $stmt = $this->conn->prepare(
            "SELECT c.id, c.cliente_id, c.servicio_id, c.estado,
                    ...
                    s.titulo, s.prestador_id, s.unidad_precio,
                    u.nombre AS prestador_nombre,
                    r.id         AS reseña_id,
                    r.puntuacion,
                    r.comentario,
                    r.fecha      AS fecha_reseña
             FROM contratos c
             JOIN servicios s ON s.id = c.servicio_id
             JOIN usuarios  u ON u.id = s.prestador_id
             LEFT JOIN reseñas r ON r.contrato_id = c.id
             WHERE c.id = ?
             LIMIT 1"
        );
        $stmt->execute([$contrato_id]);
    ...
    /**
     * Total de reseñas recibidas por un prestador.
     */
    public function getTotalReseñasRecibidas(int $prestador_id): int {
        $stmt = $this->conn->prepare(
            "SELECT COUNT(*) AS total FROM reseñas WHERE revisado_id = ?"
        );
    ...
    /**
     * Puntuación media (1 decimal) y total de reseñas recibidas por un prestador.
     */
    ...
    /**
     * Listado de reseñas recibidas (con datos del revisor y del servicio).
     */
    public function getReseñasRecibidas(int $prestador_id, int $limite = 20): array {
        $stmt = $this->conn->prepare(
            "SELECT r.id, r.puntuacion, r.comentario, r.fecha,
                    ...
                    s.id         AS servicio_id
             FROM reseñas r
             JOIN usuarios  u ON u.id = r.revisor_id
             JOIN contratos c ON c.id = r.contrato_id
             JOIN servicios s ON s.id = c.servicio_id
             WHERE r.revisado_id = ?
